...
func LoggerWithConfig(config LoggerConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
			...
			if mediaType == "application/json" {
				if v, ok := c.Get(util.ResBodyKey); ok {
					if b, ok := v.([]byte); ok && len(b) <= config.MaxOutputResponseBodyLen {
						fileds = append(fileds, zap.String("body", string(b)))
					}
				}
			}
		}
		...
		fileds = append(fileds, zap.Int64("cost", cost))
		fileds = append(fileds, zap.Int("status", c.Writer.Status()))
		fileds = append(fileds, zap.String("res_time", time.Now().Format("2006-01-02 15:04:05.999")))
		fileds = append(fileds, zap.Int("res_size", c.Writer.Size()))

		if v, ok := c.Get(util.ResBodyKey); ok {
			if b, ok := v.([]byte); ok && len(b) <= config.MaxOutputResponseBodyLen {
				fileds = append(fileds, zap.String("res_body", string(b)))
			}
		}
		...
		logging.Context(ctx).Info(fmt.Sprintf("[HTTP] %s-%s-%d (%dms)",
			c.Request.URL.Path, c.Request.Method, c.Writer.Status(), cost),
			fileds...)
	}
}
