...
func (a *JWTAuther) GenerateToken(ctx context.Context, subject string) (TokenInfo, error) {
	now := time.Now()
...
func (a *JWTAuther) parseToken(tokenStr string) (*jwt.StandardClaims, error) {
	var (
		token *jwt.Token
		err   error
	)

	for _, keyFun := range a.opts.keyFuncs {
		token, err = jwt.ParseWithClaims(tokenStr, &jwt.StandardClaims{}, keyFun)
...
func (a *JWTAuther) callStore(fn func(Storer) error) error {
...
func (a *JWTAuther) DestroyToken(ctx context.Context, tokenStr string) error {
...
func (a *JWTAuther) ParseSubject(ctx context.Context, tokenStr string) (string, error) {
	if tokenStr == "" {
		return "", nil
	}
...
func (a *JWTAuther) Release(ctx context.Context) error {
	return a.callStore(func(store Storer) error {
