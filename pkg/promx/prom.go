...
func (p *PrometheusWrapper) init() {
	...
	p.summaryLatency = prometheus.NewSummaryVec(
		prometheus.SummaryOpts{
			...
			Help:       "Summary of latencies for requests",
	...
	p.gaugeState = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "gauge_state",
			Help: "Gauge of state for requests",
		},
		[]string{"app", "module", "api"},
	)
	...
	p.counterSiteEvent = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Total number of site events",
		},
...
func (p *PrometheusWrapper) run() {
	...
	go func() {
		handler := promhttp.HandlerFor(p.reg, promhttp.HandlerOpts{})
...
func (p *PrometheusWrapper) Log(api, method, code string, sendBytes, rcvBytes, latency float64) {
	if !p.c.Enable {
		return
	}
	...
	p.counterRequests.WithLabelValues(p.c.App, "self", api, method).Inc()
	if sendBytes > 0 {
		p.counterSendBytes.WithLabelValues(p.c.App, "self", api, method, code).Add(sendBytes)
	}
	if rcvBytes > 0 {
		p.counterSendBytes.WithLabelValues(p.c.App, "self", api, method, code).Add(rcvBytes)
	}

	if len(p.c.Buckets) > 0 {
		p.histogramLatency.WithLabelValues(p.c.App, "self", api, method, code).Observe(latency)
	}

	if len(p.c.Objectives) > 0 {
		p.summaryLatency.WithLabelValues(p.c.App, "self", api, method, code).Observe(latency)
	}
}
...
func (p *PrometheusWrapper) RequestLog(module, api, method, code string) {
	...
	p.counterRequests.WithLabelValues(p.c.App, module, api, method).Inc()
}

func (p *PrometheusWrapper) SendBytesLog(module, api, method, code string, bytes float64) {
	if !p.c.Enable {
		return
	}
	p.counterSendBytes.WithLabelValues(p.c.App, module, api, method, code).Add(bytes)
}

func (p *PrometheusWrapper) RcvdBytesLog(module, api, method, code string, bytes float64) {
	if !p.c.Enable {
		return
	}
	p.counterRcvdBytes.WithLabelValues(p.c.App, module, api, method, code).Add(bytes)
}

func (p *PrometheusWrapper) HistogramLatencyLog(module, api, method, code string, latency float64) {
	if !p.c.Enable {
		return
	}
	p.histogramLatency.WithLabelValues(p.c.App, module, api, method, code).Observe(latency)
}

func (p *PrometheusWrapper) SummaryLatencyLog(module, api, method, code string, latency float64) {
	if !p.c.Enable {
		return
	}
	p.summaryLatency.WithLabelValues(p.c.App, module, api, method, code).Observe(latency)
}

func (p *PrometheusWrapper) ExceptionLog(module, api, method, code string) {
	if !p.c.Enable {
		return
	}
	p.counterException.WithLabelValues(p.c.App, module, api, method, code).Inc()
}
...
func (p *PrometheusWrapper) StateLog(module, api, method, code string, state float64) {
	if !p.c.Enable {
		return
	}
	p.gaugeState.WithLabelValues(p.c.App, module, api, method, code).Set(state)
}
